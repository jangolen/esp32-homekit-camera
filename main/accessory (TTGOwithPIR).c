...
#ifndef GPIO_PIR
#error GPIO_PIR is not specified
#endif
...
void mssd1306_init()
{
    ...
    */
    ...
    ssd1306_clearScreen();
    ssd1306_printFixed(0, 28, "HomeKit camera", STYLE_BOLD);
...
void pir_task(void *p)
{
    ...
    while ((1)) {
        ...
        if (prev != level) {
	    motion_detected.value = HOMEKIT_BOOL(level ? 1 : 0);
	    homekit_characteristic_notify(&motion_detected, motion_detected.value);
            ssd1306_clearScreen();
            if (level) {
                ESP_LOGI("PIR", "pir tirgger \n");
                ssd1306_printFixed(0, 20, "motion detected", STYLE_NORMAL);
            } 
            prev = level;
        }
...
void motion_identify(homekit_value_t _value) {
    ssd1306_clearScreen();
    ssd1306_printFixed(0, 28, "motion Identify", STYLE_BOLD);;
}
...
void camera_identify(homekit_value_t _value) {
    ...
    ssd1306_clearScreen();
    ssd1306_printFixed(0, 28, "Camera identify", STYLE_BOLD);
...
#ifdef CONFIG_CAMERA_MODEL_TTGO_V1_7
    mssd1306_init();
    xTaskCreatePinnedToCore(&pir_task, "pir_task", 2048, NULL, 5, NULL, 0);
#endif
